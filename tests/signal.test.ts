...
import { derive, dispose, antenna, LiveSignal, mutable } from "../src";

describe("mutable signal - primitive values", () => {
  ...
  it("should not trigger antennas if value is unchanged", () => {
    const count = mutable(0);
    let antennaRunCount = 0;
    ...
    expect(antennaRunCount).toBe(2); // Initial run + update

    count.value = 1; // Same value
    expect(antennaRunCount).toBe(2); // no new run
  });

  it("should have type property", () => {
    const count = mutable(0);
    expect(count.type).toBe("mutable-signal");
  });

  it("should handle null values", () => {
    const nullSignal = mutable(null);
    expect(nullSignal.value).toBe(null);
  });

  it("should handle undefined values", () => {
    const undefinedSignal = mutable(undefined);
...
describe("mutable signal - array values", () => {
  ...
  it("should trigger antennas on array mutations", () => {
    const arr = mutable([1, 2, 3]);
    let antennaRunCount = 0;
    ...
    arr.push(4);
    expect(antennaRunCount).toBe(2); // Initial + update
  });
...
describe("mutable signal - object values", () => {
  it("should create a signal with object value", () => {
    const obj = mutable({ name: "test", count: 0 });
  ...
  it("should have 'keys' method returning derived signal", () => {
    const obj = mutable({ name: "test", count: 0 } as {
      name: string;
      count: number;
      active?: boolean;
    });
  ...
  it("should trigger antennas on set", () => {
    const obj = mutable({ name: "test", count: 0 });
    let antennaRunCount = 0;
...
describe("antenna", () => {
  ...
  it("should track multiple signals", () => {
    let runCount = 0;
    const count1 = mutable(0);
    const count2 = mutable(0);
  ...
  it("should only re-run antenna if signal value was accessed", () => {
    let runCount = 0;
    const count = mutable(0);
  ...
  it("should only re-run antenna if signal value inside a conditional block was accessed in antenna's first run", () => {
    let runCount = 0;
    const count = mutable(0);
    const shouldAccess = mutable(true);
    ...
    count.value = 2;
    expect(runCount).toBe(4); // count change still triggers re-run (antenna is still registered to count)
  });

  it("should skip running antenna if signal value inside a conditional block was missed in antenna's first run", () => {
    let runCount = 0;
    const count = mutable(0);
    const shouldAccess = mutable(false);
  ...
  it("should have isDisposed property", () => {
    const count = mutable(0);
    const receiver = antenna(() => {
      count.value;
    });

    expect(receiver.isDisposed).toBe(false);
  });
  ...
  it("should have dispose method", () => {
    ...
    receiver.dispose();
    expect(receiver.isDisposed).toBe(true);
  });
  ...
  it("should be removed from signal on next update after disposal", () => {
    ...
    receiver.dispose();
    count.value = 1; // This should remove the antenna from the signal's antenna set without running it
    count.value = 2; // This should not trigger the disposed antenna
    expect(runCount).toBe(1); // Only initial run
  });
});
...
describe("derive", () => {
  ...
  it("should have prevValue getter", () => {
    ...
    // prevValue is set during initial antenna run
...
describe("dispose utility", () => {
  ...
  it("should dispose single antenna", () => {
    let runCount = 0;
    const count = mutable(0);
  ...
  it("should dispose multiple antennas", () => {
    let runCount1 = 0;
    let runCount2 = 0;
    const count = mutable(0);

    const receiver1 = antenna(() => {
      runCount1++;
      count.value;
    });

    const receiver2 = antenna(() => {
      runCount2++;
      count.value;
    });

    dispose(receiver1, receiver2);
    count.value = 5;
    expect(runCount1).toBe(1);
    expect(runCount2).toBe(1);
  });

  it("should dispose mixed signals and antennas", () => {
    let runCount = 0;
    const count = mutable(0);
    const doubled = derive(() => count.value * 2);
...
describe("mutable signal - string values", () => {
  ...
  it("should have 'replaceAll' method returning derived signal", () => {
    const text = mutable("hello hello");
...
describe("mutable signal - number values", () => {
...
describe("mutable signal - boolean values", () => {
...
describe("mutable signal - nullable types", () => {
  it("should handle string | undefined with undefined initial value", () => {
    const text = mutable<string | undefined>(undefined, "");
    expect(text.value).toBeUndefined();
    // Check if methods are available even when value is undefined
    expect(typeof (text as LiveSignal<string>).length).toBe("function");
    expect(typeof (text as LiveSignal<string>).lowercase).toBe("function");
    expect(typeof (text as LiveSignal<string>).UPPERCASE).toBe("function");
  });
  ...
  it("should handle number | undefined with undefined initial value", () => {
    const num = mutable<number | undefined>(undefined, 0);
    expect(num.value).toBeUndefined();
    // Check if methods are available even when value is undefined
    expect(typeof (num as LiveSignal<number>).toFixed).toBe("function");
    expect(typeof (num as LiveSignal<number>).toPrecision).toBe("function");
    expect(typeof (num as LiveSignal<number>).toExponential).toBe("function");
  });

  it("should handle number | null with null initial value", () => {
    const num = mutable<number | null>(null, 0);
  ...
  it("should handle array | null with null initial value", () => {
    const arr = mutable<number[] | null>(null, []);
  ...
  it("should handle object | undefined with undefined initial value", () => {
    const obj = mutable<{ name: string } | undefined>(
      undefined,
      {} as { name: string },
    );
  ...
  it("should handle object | null with null initial value", () => {
    const obj = mutable<{ name: string } | null>(null, {} as { name: string });
  ...
  it("should handle string | undefined transitioning from undefined to string", () => {
    ...
    expect(text.value).toBeUndefined();
    text.value = "hello";
    const length = (text as LiveSignal<string>).length();
    expect(text.value).toBe("hello");
    expect(length.value).toBe(5);
  });
  ...
  it("should handle number | undefined transitioning from undefined to number", () => {
    ...
    expect(num.value).toBeUndefined();
    num.value = 3.14159;
    const fixed = (num as LiveSignal<number>).toFixed(2);
